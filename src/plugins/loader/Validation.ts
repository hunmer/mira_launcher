...
export class PluginValidator {
  ...
  private initializeDefaultRules(): void {
    ...
    this.rules.set(
      ...
      this.validateVersionFormat.bind(this),
    )
    this.rules.set('metadata.id-format', this.validateIdFormat.bind(this))

    // 兼容性验证规则
    this.rules.set(
      'compatibility.app-version',
      this.validateAppVersion.bind(this),
    )
    this.rules.set(
    ...
    this.rules.set(
      'security.dangerous-permissions',
      this.validateDangerousPermissions.bind(this),
    )

    // 插件类验证规则
    this.rules.set(
      'plugin.class-structure',
      this.validatePluginClass.bind(this),
    )
    this.rules.set(
      'plugin.lifecycle-methods',
      this.validateLifecycleMethods.bind(this),
    )

    // 文件系统验证规则
    this.rules.set('files.entry-exists', this.validateEntryFile.bind(this))
    this.rules.set(
      'files.manifest-integrity',
      this.validateManifestIntegrity.bind(this),
    )
  }
   ...
   * 验证元数据格式
   */
  private validateMetadataFormat(
    target: any,
    context: ValidationContext,
  ): ValidationResult {
    const metadata = context.metadata || target.metadata
  ...
  private validateIdFormat(
    ...
    if (!idRegex.test(id)) {
      return {
        ...
        severity: 'error',
        message:
          'Plugin ID can only contain lowercase letters, numbers, hyphens, and underscores',
        rule: 'metadata.id-format',
        details: { id },
      }
    }
  ...
  private validateAppVersion(
    ...
    // 简单的版本比较
    const isCompatible =
      this.compareVersions(context.appVersion, minAppVersion) >= 0
  ...
  private validatePermissions(
    ...
    const invalidPermissions = permissions.filter(
      (p: string) => !context.allowedPermissions.includes(p),
    )
   ...
   * 验证危险权限
   */
  private validateDangerousPermissions(
    target: any,
    context: ValidationContext,
  ): ValidationResult {
    const metadata = context.metadata || target.metadata
    ...
    const hasDangerous = permissions.some((p: string) =>
    ...
    if (hasDangerous && context.mode === 'strict') {
      return {
        ...
        details: {
          dangerousPermissions: permissions.filter((p: string) =>
            dangerousPermissions.includes(p),
          ),
        },
      }
   ...
   * 验证生命周期方法
   */
  private validateLifecycleMethods(
    target: any,
    context: ValidationContext,
  ): ValidationResult {
    const loadResult = context.loadResult
    ...
    if (!loadResult?.pluginClass) {
      return {
        ...
        severity: 'error',
        message:
          'Cannot validate lifecycle methods: plugin class not available',
 ...
 * 创建插件验证器实例
 */
export function createPluginValidator(
...
export const validationUtils = {
   ...
   * 快速验证插件元数据
   */
  quickValidateMetadata(metadata: PluginMetadata): {
    valid: boolean
    errors: string[]
  } {
    const errors: string[] = []
