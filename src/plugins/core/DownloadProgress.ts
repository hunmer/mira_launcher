...
export class BrowserDownloadHelper {
  ...
  private async fetchFileInfo(): Promise<void> {
    try {
      ...
      console.warn(
        'HEAD request failed, will determine size during download:',
        error,
      )
    }
  }
...
export class DownloadProgressManager {
  ...
  getStats(): {
    ...
    const downloadedBytes = progressArray.reduce(
      (sum, stats) => sum + stats.downloaded,
      0,
    )
    const averageSpeed =
      progressArray.length > 0
        ? progressArray.reduce((sum, stats) => sum + stats.speed, 0) /
