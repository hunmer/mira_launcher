<template>
  <div class="space-y-6">
    <SettingPlan :default-value="['0', '1', '2']">
      <!-- 系统快捷键设置 -->
      <SettingCard value="0" title="系统快捷键" icon="pi pi-key" icon-color="text-yellow-600 dark:text-yellow-400">
        <!-- 自定义快捷键设置组件 -->
        <div class="space-y-4">
          <ShortcutSettingItem title="全局唤醒快捷键" tooltip="快速显示/隐藏启动器窗口的全局快捷键"
            v-model="settingsStore.settings.shortcuts.globalHotkey" :shortcut-type="'global'" :show-delete-button="true"
            :is-built-in="true" @change="saveShortcutSetting('globalHotkey', $event)"
            @delete="saveShortcutSetting('globalHotkey', '')" />

          <ShortcutSettingItem title="搜索快捷键" tooltip="启动搜索功能的快捷键"
            v-model="settingsStore.settings.shortcuts.searchHotkey" :shortcut-type="'application'"
            :show-delete-button="true" :is-built-in="true" @change="saveShortcutSetting('searchHotkey', $event)"
            @delete="saveShortcutSetting('searchHotkey', '')" />

          <ShortcutSettingItem title="快速搜索快捷键" tooltip="打开快速搜索面板"
            v-model="settingsStore.settings.shortcuts.quickSearchHotkey" :shortcut-type="'application'"
            :show-delete-button="true" :is-built-in="true" @change="saveShortcutSetting('quickSearchHotkey', $event)"
            @delete="saveShortcutSetting('quickSearchHotkey', '')" />

          <ShortcutSettingItem title="设置页面快捷键" tooltip="打开设置页面的快捷键"
            v-model="settingsStore.settings.shortcuts.settingsHotkey" :shortcut-type="'application'"
            :show-delete-button="true" :is-built-in="true" @change="saveShortcutSetting('settingsHotkey', $event)"
            @delete="saveShortcutSetting('settingsHotkey', '')" />

          <ShortcutSettingItem title="首页快捷键" tooltip="快速跳转到首页" v-model="settingsStore.settings.shortcuts.homeHotkey"
            :shortcut-type="'application'" :show-delete-button="true" :is-built-in="true"
            @change="saveShortcutSetting('homeHotkey', $event)" @delete="saveShortcutSetting('homeHotkey', '')" />

          <ShortcutSettingItem title="应用管理快捷键" tooltip="快速跳转到应用管理页面"
            v-model="settingsStore.settings.shortcuts.applicationsHotkey" :shortcut-type="'application'"
            :show-delete-button="true" :is-built-in="true" @change="saveShortcutSetting('applicationsHotkey', $event)"
            @delete="saveShortcutSetting('applicationsHotkey', '')" />

          <ShortcutSettingItem title="插件管理快捷键" tooltip="快速跳转到插件管理页面"
            v-model="settingsStore.settings.shortcuts.pluginsHotkey" :shortcut-type="'application'"
            :show-delete-button="true" :is-built-in="true" @change="saveShortcutSetting('pluginsHotkey', $event)"
            @delete="saveShortcutSetting('pluginsHotkey', '')" />

          <ShortcutSettingItem title="退出应用快捷键" tooltip="退出应用程序的快捷键"
            v-model="settingsStore.settings.shortcuts.exitHotkey" :shortcut-type="'global'" :show-delete-button="true"
            :is-built-in="true" @change="saveShortcutSetting('exitHotkey', $event)"
            @delete="saveShortcutSetting('exitHotkey', '')" />
        </div>
      </SettingCard>

      <!-- 自定义快捷键管理 -->
      <SettingCard value="1" title="自定义快捷键" icon="pi pi-plus" icon-color="text-blue-600 dark:text-blue-400">
        <!-- 添加新快捷键按钮 -->
        <div class="mb-4">
          <Button label="添加自定义快捷键" icon="pi pi-plus" size="small" @click="showAddShortcutDialog = true" />
        </div>

        <!-- 自定义快捷键列表 -->
        <div v-if="customShortcuts.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
          <i class="pi pi-keyboard text-3xl mb-2"></i>
          <p>暂无自定义快捷键</p>
          <p class="text-sm">点击上方按钮添加您的第一个自定义快捷键</p>
        </div>

        <div v-else class="space-y-4">
          <ShortcutSettingItem v-for="(shortcut, index) in customShortcuts" :key="shortcut.id"
            :title="getActionName(shortcut.actionId)" :tooltip="shortcut.description || '自定义快捷键'" v-model="shortcut.key"
            :shortcut-type="shortcut.type" :show-delete-button="true" @change="updateCustomShortcut(index, $event)"
            @delete="removeShortcut(index)" />
        </div>
      </SettingCard>

      <!-- 鼠标设置 -->
      <SettingCard value="2" title="鼠标设置" icon="pi pi-mouse" icon-color="text-teal-600 dark:text-teal-400">
        <SettingItem title="双击启动应用" tooltip="双击应用图标启动应用程序" type="text" right-control="switch"
          v-model="doubleClickToLaunch" @update:model-value="saveDoubleClickToLaunch" />

        <SettingItem title="右键菜单" tooltip="启用应用图标的右键上下文菜单" type="text" right-control="switch"
          v-model="enableContextMenu" @update:model-value="saveEnableContextMenu" />
      </SettingCard>
    </SettingPlan>

    <!-- 添加快捷键对话框 -->
    <ShortcutDialog v-model:visible="showAddShortcutDialog" mode="add" title="添加自定义快捷键"
      :available-actions="availableActions" :conflict-checker="(key) => checkShortcutConflict(key, 'application')"
      @save="handleAddShortcut" @cancel="cancelAddShortcut" />

    <!-- 编辑快捷键对话框 -->
    <ShortcutDialog v-model:visible="showEditShortcutDialog" mode="edit" title="编辑自定义快捷键"
      :initial-data="editingShortcut" :available-actions="availableActions"
      :conflict-checker="(key) => checkShortcutConflictForEdit(key, editingShortcut.type)"
      @save="handleSaveEditShortcut" @cancel="cancelEditShortcut" />
  </div>
</template>
