        ...
        <SettingPlan :default-value="['0', '1', '2']">
            ...
            >
                <SettingItem
                    ...
                    title="开机自启动"
                    tooltip="系统启动时自动运行 Mira Launcher"
                    type="text"
                ...
                <SettingItem
                    v-model="settingsStore.settings.startup.startMinimized"
                    title="最小化到系统托盘"
                    tooltip="关闭窗口时最小化到系统托盘"
                    type="text"
                    right-control="switch"
                    readonly
                />
                ...
                <SettingItem
                    v-model="settingsStore.settings.startup.loadPluginsOnStart"
                    title="启动时加载插件"
                    tooltip="应用程序启动时自动加载所有插件"
                    type="text"
                    right-control="switch"
                    readonly
                />
            ...
            >
                <SettingItem
                    ...
                    title="启用硬件加速"
                    tooltip="使用 GPU 加速渲染，提升性能"
                    type="text"
                    right-control="switch"
                    readonly
                    @update:model-value="saveHardwareAcceleration"
                />

                <SettingItem
                    v-model="preloadApps"
                    title="预加载应用信息"
                    tooltip="启动时预先加载应用程序信息，提升响应速度"
                    type="text"
                    right-control="switch"
                    readonly
                    @update:model-value="savePreloadApps"
                />
            </SettingCard>
            ...
            >
                <SettingItem
                    v-model="settingsStore.settings.startup.checkUpdates"
                    title="自动检查更新"
                    tooltip="定期检查并提示有可用更新"
                    type="text"
                    right-control="switch"
                    readonly
                />
